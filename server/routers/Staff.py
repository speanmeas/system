import os
import sys


sys.path.append(os.getcwd())


from typing import *
from fastapi import *


import re
import json
from PIL import Image
from io import BytesIO
from datetime import datetime
from rich import print as pprint
from bson import ObjectId, json_util

from Environment import *
from utilities.Database import database as db
from utilities.Storage import storage as s3
from utilities.Token import token as tk
from utilities.Converter import converter as cvt

router = APIRouter()

COLLECTION = "c_staff"

# Define staff columns
COLLUMN_STRINGS = [
    "name",
    "email",
    "position",
    "department",
]
COLLUMN_NUMBERS = [
    "phone_number",
    "salary",
]
COLLUMN_IMAGES = [
    "profile",
]


# * OK - Create new staff
def create():
    async def _():
        try:
            counter = await db["c_counter"].find_one_and_update(
                {"_id": COLLECTION},
                {"$inc": {"seq": 1}},
                upsert=True,
                return_document=True,
            )

            data = {
                **{c: None for c in COLLUMN_NUMBERS},
                **{c: None for c in COLLUMN_STRINGS},
                **{c: None for c in COLLUMN_IMAGES},
                "created_at": datetime.now(),
                "reorder": counter["seq"],
                "updated_at": None,
                "deleted_at": None,
            }

            result = await db[COLLECTION].insert_one({**data})

            return {"id": str(result.inserted_id), "success": True}
        except Exception:
            return Response(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

    return _


# * OK - Read staff with pagination
def read():
    async def _(
        query: Optional[str] = Form(None, json_schema_extra={"example": ""}),
        offset: Optional[int] = Form(None, json_schema_extra={"example": 0}, ge=0),
        limit: Optional[int] = Form(None, json_schema_extra={"example": 1000}, ge=1, le=10000),
        sort_by: Literal["created_at", "order", *COLLUMN_STRINGS, *COLLUMN_NUMBERS] = Form("created_at"),
        sort_order: Literal["-1", "1"] = Form("-1"),
    ):
        try:
            if not query:
                search = (
                    await db[COLLECTION]
                    .find(
                        {"deleted_at": {"$eq": None}},
                    )
                    .sort(sort_by, int(sort_order))
                    .skip(offset)
                    .limit(limit)
                    .to_list(length=None)
                )
                return json.loads(json_util.dumps(search))

            search = (
                await db[COLLECTION]
                .find(
                    {
                        "$and": [
                            {"$or": [{c: {"$regex": re.escape(query), "$options": "i"}} for c in [*COLLUMN_STRINGS, *COLLUMN_NUMBERS]]},
                            {"deleted_at": {"$eq": None}},
                        ]
                    }
                )
                .sort(sort_by, int(sort_order))
                .skip(offset)
                .limit(limit)
                .to_list(length=None)
            )
            return json.loads(json_util.dumps(search))
        except Exception:
            return Response(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

    return _


# * OK - Update string field
def update_string(key):
    async def _(
        id: str = Form(..., json_schema_extra={"example": ""}),
        value: Optional[str] = Form(None, json_schema_extra={"example": ""}),
    ):
        try:
            exist = await db[COLLECTION].find_one({"_id": ObjectId(id)})
            if not exist:
                return Response(status_code=status.HTTP_404_NOT_FOUND)

            await db[COLLECTION].update_one(
                {"_id": ObjectId(id)},
                {"$set": {key: value, "updated_at": datetime.now()}},
            )

            return {"success": True}
        except Exception:
            return Response(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

    return _


# * OK - Update number field
def update_number(key):
    async def _(
        id: str = Form(..., json_schema_extra={"example": ""}),
        value: Optional[float] = Form(None, json_schema_extra={"example": 0.0}),
    ):
        try:
            exist = await db[COLLECTION].find_one({"_id": ObjectId(id)})
            if not exist:
                return Response(status_code=status.HTTP_404_NOT_FOUND)

            await db[COLLECTION].update_one(
                {"_id": ObjectId(id)},
                {"$set": {key: value, "updated_at": datetime.now()}},
            )

            return {"success": True}
        except Exception:
            return Response(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

    return _


# * OK - Upload image
def upload_image(key):
    async def _(
        id: str = Form(..., json_schema_extra={"example": ""}),
        value: UploadFile = File(...),
    ):
        try:
            exist = await db[COLLECTION].find_one({"_id": ObjectId(id)})
            if not exist:
                return Response(status_code=status.HTTP_404_NOT_FOUND)

            content = await value.read()
            if len(content) > 10 * 1024 * 1024 or len(content) <= 0:
                return Response(status_code=status.HTTP_400_BAD_REQUEST)

            date_time = datetime.now().strftime("%Y%m%d%H%M%S%f")
            access_token = tk.gen(16)
            image_ext = "png"

            new_image_name = f"{date_time}_{access_token}.{image_ext}"

            old_image_name = exist.get(key)
            if old_image_name:
                if s3.object_exists(MINIO_BUCKET_PUBLIC, old_image_name):
                    s3.remove_object(MINIO_BUCKET_PUBLIC, old_image_name)

            image = Image.open(BytesIO(content))
            image_buffer = BytesIO()
            image.save(image_buffer, format="PNG")
            image_buffer.seek(0)

            s3.put_object(
                bucket_name=MINIO_BUCKET_PUBLIC,
                object_name=f"images/{new_image_name}",
                data=image_buffer,
                length=image_buffer.getbuffer().nbytes,
            )

            await db[COLLECTION].update_one(
                {"_id": ObjectId(id)},
                {"$set": {key: new_image_name, "updated_at": datetime.now()}},
            )

            cvt.to_thumbnail(f"images/{new_image_name}", 100)
            cvt.to_thumbnail(f"images/{new_image_name}", 200)
            return {"success": True, "image_name": new_image_name}
        except Exception:
            return Response(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

    return _


# * OK - Delete staff (soft delete)
def delete():
    async def _(
        id: str = Form(..., json_schema_extra={"example": ""}),
    ):
        try:
            exist = await db[COLLECTION].find_one({"_id": ObjectId(id)})
            if not exist:
                return Response(status_code=status.HTTP_404_NOT_FOUND)

            await db[COLLECTION].update_one(
                {"_id": ObjectId(id)},
                {"$set": {"deleted_at": datetime.now()}},
            )

            return {"success": True}
        except Exception:
            return Response(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

    return _


# * OK - Delete specific field
def delete_field(key):
    async def _(
        id: str = Form(..., json_schema_extra={"example": ""}),
    ):
        try:
            exist = await db[COLLECTION].find_one({"_id": ObjectId(id)})
            if not exist:
                return Response(status_code=status.HTTP_404_NOT_FOUND)

            await db[COLLECTION].update_one(
                {"_id": ObjectId(id)},
                {"$set": {key: None, "updated_at": datetime.now()}},
            )

            return {"success": True}
        except Exception:
            return Response(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

    return _


# Routes
router.post("/create", deprecated=0)(create())
router.post("/read", deprecated=0)(read())

# Update routes for string fields
for c in COLLUMN_STRINGS:
    router.post(f"/update/{c}", deprecated=0)(update_string(c))

# Update routes for number fields
for c in COLLUMN_NUMBERS:
    router.post(f"/update/{c}", deprecated=0)(update_number(c))

# Upload routes for image fields
for c in COLLUMN_IMAGES:
    router.post(f"/upload/{c}", deprecated=0)(upload_image(c))

# Delete routes
router.post("/delete", deprecated=0)(delete())
for c in [*COLLUMN_STRINGS, *COLLUMN_NUMBERS, *COLLUMN_IMAGES]:
    router.post(f"/delete/{c}", deprecated=0)(delete_field(c))


if __name__ == "__main__":
    os.system("python Main.py")
